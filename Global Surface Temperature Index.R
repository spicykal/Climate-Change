data <- read.csv(url("http://data.giss.nasa.gov/gistemp/graphs/graph_data/mixedGLB.Ts.ERSSTV4.GHCN.CL.PA.csv"), skip = 1)
library(dplyr)
...
ggplot(data, aes(Year, y = value, color = variable)) + 
  ...
  geom_line(aes(y = Dec, col = "Dec")) + ggtitle("Global Surface Temperature Difference (Base Period: 1951-1980)")+
  xlab("Years") +
